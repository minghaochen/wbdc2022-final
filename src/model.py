...
#             nn.GELU(),
        ...
        self.class_text = nn.Linear(bert_output_size, len(CATEGORY_ID_LIST))
        # self.class_video = nn.Linear(1024, len(CATEGORY_ID_LIST))
        self.class_video = nn.Linear(bert_output_size, len(CATEGORY_ID_LIST))
    ...
    def forward(self, inputs, inference=False):
        ...
        # inputs['frame_input'] = self.visual_backbone_proj(inputs['frame_input']) # 位置需要改一下
        ...
        subtask1_pred = self.class_text(self.cross_modal_text_pooler2(text_embeds, inputs['title_mask']))
...
#         prediction = self.classifier(final_embedding)
    ...
    def cal_loss(prediction, label, subtask1_pred, subtask2_pred,
        ...
        loss = loss_main+0.25*loss_sub1+0.25*loss_sub2 # TODO: KLD
        with torch.no_grad():
