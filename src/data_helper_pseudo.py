...
def create_dataloaders(args):
    ...
    # skf dataset
    train_dataset = MultiModalDataset(args, args.train_annotation, args.test_annotation, args.unlabel_path,
                                      args.train_zip_frames, args.test_zip_frames, fold=args.fold, get_skf='train')
    val_dataset = MultiModalDataset(args, args.train_annotation, args.test_annotation, args.unlabel_path,
                                    args.train_zip_frames, args.test_zip_frames, fold=args.fold, get_skf='valid')
...
class MultiModalDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 ann_path: str,
                 ## 对应无标签的ann地址
                 unlabel_ann_path: str,
                 ## 推理部分输出的文件地址
                 unlabel_pred_path: str,
                 ## label的frams地址
                 zip_frame_dir: str,
                 ## unlabel的frames地址
                 unlabel_zip_frame_dir:str,
                 fold: int = 0,
                 get_skf: str = 'train',
    ...
    #              get_skf: str = 'train'):
        ...
        self.unlabel_zip_frame_dir = unlabel_zip_frame_dir
        ...
        with open(unlabel_ann_path, 'r' , encoding='utf8') as f:
            self.unlabel_anns = json.load(f)
        
        with open(unlabel_pred_path, 'r' , encoding='utf8') as f:
            self.unlabel_preds = json.load(f)
        ...
        unlabel_anns = []
        self.unlabel_preds = sorted(self.unlabel_preds, key=lambda x:x['prob'], reverse=True) 
        for unlabel_pred in self.unlabel_preds:
            ann = self.unlabel_anns[unlabel_pred['idx']]
            ann['category_id'] = unlabel_pred['category_id']
            ann['flag'] = 'unlabel'
            unlabel_anns.append(ann)
            
        # with open(ann_path, 'r', encoding='utf8') as f:
        #     self.anns = json.load(f)
        length2 = len(unlabel_anns)
        
        print('label', length1, 'unlabel', length2)
        # self.anns = anns[-500:] + anns[:500]
        self.anns = anns
...
#             self.anns[j]['raw_text'] += ocr
    ...
    def get_visual_frames(self, idx: int) -> tuple:
        ...
        flag = self.anns[idx]['flag']
        
        if (flag == 'label'):
            zip_path = os.path.join(self.zip_frame_dir, f'{vid[-3:]}/{vid}.zip')
        elif (flag == 'unlabel'):
            zip_path = os.path.join(self.unlabel_zip_frame_dir, f'{vid[-3:]}/{vid}.zip')
