...
def train_and_validate(args):
    ...
    checkpoint = torch.load('save/v1/model_fold_0_epoch_2_mean_f1_0.7125.bin', map_location='cpu')
...
#             save_step += 1
#             if (best_score>=0.69) and (save_step % 1001 == 0):
#                 # ema.store()
#                 # ema.copy_to()
...
#         logging.info(f"Epoch {epoch} step {step}: loss {loss:.3f}, {results}")
    ...
    print('infer QPS:', len(val_dataloader)*args.val_batch_size/(infer_end-infer_begin))
...
#             state_dict = model.module.state_dict() if args.device == 'cuda' else model.state_dict()
#             # torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'mean_f1': mean_f1},
#                        # f'{args.savedmodel_path}/model_fold_{args.fold}.bin')
#             torch.save({'epoch': epoch, 'model_state_dict': state_dict, 'mean_f1': mean_f1},
#                        f'{args.savedmodel_path}/model_fold_{args.fold}_epoch_{epoch}_mean_f1_{mean_f1}.bin')
            
                
#             np.save(f'{args.savedmodel_path}/raw_predictions_valid.npy', raw_predictions)
#             np.save(f'{args.savedmodel_path}/labels_valid.npy', labels)
    
#             ema.restore()
#         else:
#             del train_dataloader, val_dataloader, model, ema, fgm
