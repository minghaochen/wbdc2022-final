...
class MultiModalDataset(Dataset):
    ...
    def tokenize_text2(self, title: str, ocr_text: str, asr_text: str) -> tuple:
        if len(title) >= 80:
            title = title[:40] + title[-40:]
        if len(ocr_text) >= 80:
            ocr_text = ocr_text[:40] + ocr_text[-40:]
        if len(asr_text) >= 80:
            asr_text = asr_text[:40] + asr_text[-40:]

        encoded_titles = self.tokenizer(title, max_length=80, padding='max_length', truncation=True)
        encoded_ocr = self.tokenizer(ocr_text, max_length=80, padding='max_length', truncation=True)
        encoded_asr = self.tokenizer(asr_text, max_length=80, padding='max_length', truncation=True)
    ...
    def tokenize_text3(self, title: str, ocr_text: str, asr_text: str) -> tuple:
        ...
        text = '[CLS]' + '[SEP]' + title + '[SEP]' + ocr_text + '[SEP]' + asr_text + '[SEP]'
        
        encoded_inputs = self.tokenizer(text, max_length=260, padding='max_length', 
