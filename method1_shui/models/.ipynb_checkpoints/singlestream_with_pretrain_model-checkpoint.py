...
class myClassifer(nn.Module):
    def __init__(self, uni_bert_cfg, dropout = [0.2, 0.2], hidden_layers = 768,  n_weights=24):
        super().__init__()
    ...
    def forward(self, hidden_states):
        cls_outputs = torch.stack(
            [self.dropout(layer) for layer in hidden_states[-24:]], dim=0
        )
        cls_output = (
...
#         self.dropout5 = nn.Dropout(0.5)
    ...
    def forward(self, inputs, inference=False):
        ...
        prediction = self.classify_dense(self.dropout(output))
...
#         self.dropout5 = nn.Dropout(0.5)
        ...
        ## 任务层部分
        self.my_classifer = myClassifer(uni_bert_cfg, dropout = [0.2, 0.2], hidden_layers = 768,  n_weights=24)
        self.dropout = nn.Dropout(0.2)
        # self.my_classifer = MeanPooling()
        self.classify_dense = nn.Linear(uni_bert_cfg.hidden_size, len(CATEGORY_ID_LIST))
...
class UniBert4(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # self.video_fc = nn.Linear(1024, config.hidden_size)
        # nn.Sequential(
    ...
    def forward(self, video_feature, video_mask, text_input_ids, text_mask):
        ...
        return encoder_outputs, mask
# bert base + swim base
class UniBert5(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
...
### 初赛去掉 videodense   
class UniBert6(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
